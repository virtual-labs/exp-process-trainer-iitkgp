...
//---------------------------------------------------------output wave for 40 degree openning,100%PB, 279 mm,set value 40 degree centigrate,input sine wave 2v-------------------------------------------------------------------------------//
	var dataOPPoints=[];
    var x = new Array(), y = new Array();  // x,y plotting variables
    var dt, tstart, tstop,tperiod,cyclehlfs,tmpv,count=50; 
	var measured;
...
if(document.getElementById('P').value==50){	
    ...
    /* for (var t = 0; t < axes.N; t++) {
        ...
        x[t] = tstart + t * dt;
        y[t] = (out/2) * Math.sign( Math.sin(2 * 3.1415 * frqfng * x[t] + ((phsl * 3.1415 / 180)+ out2)));
	 ...
	 */
	 ...
	 /////////////////////////////////////////
		tmpv =  math.divide(document.getElementById('fq-knob').value,1000);
		tperiod = math.divide(1,frqfng);///in sec
		var factor = math.divide(tperiod,tmpv);
		cyclehlfs = math.multiply(math.divide(10,factor),2);

	 var td1 = (document.getElementById('fq-knob').value*10);
	 var td2 = math.multiply(td,1000);
	 var divcal = math.number(math.multiply(math.divide(50,td1),td2,1.1));///factor 1.1 is additional to adjust stroke length
	  var ctxcal = math.multiply(8.4,math.round(divcal));
	 ///test ctx draw
	 const canvas = document.getElementById("mycanvas");
		...
		ctx.lineTo(ctxcal,canvas.height/2);
		ctx.lineWidth = 2;
		ctx.strokeStyle = "red";
		ctx.stroke();  
	...
	/*for(var t = 0; t<=td; t+=0.1){
		x[t]=(tstart+td) + t*dt;
		y[t]= 0;
		dataOPPoints.push({x:(t), y:(y[t])});
	}*/
	/*for(var t = math.multiply(math.divide(200,td1),td2); t<=math.multiply(2,math.add(nc,0.01),tperiod,10); t+=0.1){
		x[t]=tstart + t*dt;
		y[t]= 0;
		dataOPPoints.push({x:(t), y:(y[t])});
	}*/
	//for(var nc = 0; nc< math.divide(cyclehlfs,2); nc+=1){		
    for (var t = math.round(math.multiply(math.add(nc,0),math.divide(tperiod,tmpv),10)); t <= math.round(math.multiply(math.add(math.multiply(nc,2),1),math.divide(axes.N,cyclehlfs))); t++) {
	...
	dataOPPoints.push({x:(t), y:(y[t])});
		...
		//for (var nc = 0; nc < math.divide(cyclehlfs,2); nc+=1) {
	/* for(var t = 0; t<=divcal; t+=0.1){
		x[t]=0;//tstart + t*dt;
		y[t]= 0;
		dataOPPoints.push({x:(t), y:(y[t])});
	} */
	/* for(var nc = 0; nc< math.divide(cyclehlfs,2); nc+=1){		
    for (var t = math.add(math.multiply(2,nc,tperiod),divcal); t <= math.round(math.add(math.multiply(math.add(math.multiply(nc,2),1),math.divide(axes.N,cyclehlfs)),divcal)); t++) {
	...
	//for (var t = 50; t < (axes.N); t++) {
		for (var t = math.round(math.add(math.multiply(math.add(math.multiply(nc,2),1),math.divide(axes.N,cyclehlfs)),divcal)); t<= math.round(math.add(math.multiply(2,math.divide(axes.N,cyclehlfs),math.add(((2*nc)/2),1)),divcal)); t++) {
...
//---------------------------------------------------------Square wave-------------------------------------------------------------------------------//
	...
	var midchange = math.add(50,T2);
	 ...
	 /////////////////////////////////////////
		var tmpv =  math.divide(document.getElementById('fq-knob').value,1000);
		 tperiod = math.divide(1,frqfng);///in sec
		var factor = math.divide(tperiod,tmpv);
		cyclehlfs = math.multiply(math.divide(10,factor),2);

	 var td1 = (document.getElementById('fq-knob').value*10);
	 var td2 = math.multiply(T2,1000);
	 var divcal = math.number(math.multiply(math.divide(50,td1),td2,1.1));
	  var ctxcal = math.multiply(8.4,math.round(divcal));
	 ///test ctx draw
	 const canvas = document.getElementById("mycanvas");
		const ctx = canvas.getContext("2d");
...
if(document.getElementById('P').value==50){	
	...
	if(document.getElementById('controllerchk').value ==3){
	var sserr = math.multiply(math.divide(math.abs(math.subtract(vp,measured)),vp),100);
	document.getElementById('sserr').value = "";
	document.getElementById('tpl').value = math.multiply(td,1000);
	document.getElementById('tfl').value = math.multiply(tau,1000);
	}
	if(document.getElementById('dischk').value==1){
...
if(document.getElementById('P').value==100){	
	...
	var td = 0.120;    ///delay time,from lab test or transport lag
		...
		/* var td1 = (document.getElementById('fq-knob').value*10);
	 var td2 = math.multiply(td,1000);
	 var divcal = math.number(math.multiply(math.divide(50,td1),td2));
	  var ctxcal = math.multiply(8.4,math.round(divcal));
	 ///test ctx draw
	 const canvas = document.getElementById("mycanvas");
		const ctx = canvas.getContext("2d");
